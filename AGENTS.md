...
- `Store.effectProcess` прогоняет каждый эффект через каждый обработчик из `effectHandlers`, поэтому массив из нескольких обработчиков даёт дублирование работы. Рабочий путь — ровно один `effectHandler`.
